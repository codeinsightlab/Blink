...
const primaryLinks = [
  ...
  { to: "/setup", label: "能力配置", icon: SlidersHorizontal },
];
const advancedLinks = [
  { to: "/apps", label: "软件库", icon: AppWindow },
  { to: "/commands", label: "命令库", icon: TerminalSquare },
  { to: "/mapping", label: "能力列表", icon: Keyboard },
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="fixed inset-y-0 left-0 w-60 border-r border-slate-800 bg-slate-950 p-5 text-slate-300">
          ...
          <div className="space-y-1">
            {primaryLinks.map(({ to, label, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${isActive ? "bg-indigo-600 text-white" : "hover:bg-slate-900 hover:text-white"}`
                }
              >
                <Icon size={18} />
                {label}
          ...
          <div className="space-y-1">
            {advancedLinks.map(({ to, label, icon: Icon }) => (
              <NavLink
                ...
                to={to}
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${isActive ? "bg-slate-800 text-white" : "hover:bg-slate-900 hover:text-white"}`
                }
              >
                <Icon size={18} />
                {label}
        ...
        <div className="absolute bottom-5 left-5 right-5 rounded-lg border border-slate-800 p-3 text-xs leading-5 text-slate-500">
          仅限本地配置
          <br />
          运行端尚未连接
        </div>
