...
const categoryTranslations: Record<string, string> = { Browser: "浏览器", Communication: "沟通", Office: "办公", Creative: "创意", Developer: "开发" };
function localizeCategories(apps: AppDefinition[]) {
  return apps.map((app) => ({ ...app, category: app.category ? (categoryTranslations[app.category] ?? app.category) : app.category }));
}
...
const persist = (apps: AppDefinition[]) => { writeStorage(STORAGE_KEYS.apps, apps); return apps; };
export const useAppRegistryStore = create<RegistryState>((set) => ({
  apps: loadApps(),
  upsertApp: (app) => set((state) => ({ apps: persist(state.apps.some((item) => item.id === app.id) ? state.apps.map((item) => item.id === app.id ? app : item) : [...state.apps, app]) })),
  deleteApp: (id) => set((state) => ({ apps: persist(state.apps.filter((app) => app.id !== id)) })),
  toggleApp: (id) => set((state) => ({ apps: persist(state.apps.map((app) => app.id === id ? { ...app, enabled: !app.enabled, updatedAt: new Date().toISOString() } : app)) })),
}));
