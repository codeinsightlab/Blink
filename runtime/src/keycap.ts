...
const STATE_LABEL: Record<HotkeyState, string> = { conflict: "冲突", "registration-failed": "注册失败", paused: "已暂停" };

const escapeHtml = (value: string) => value.replace(/[&<>\"]/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[character]!);

export function Keycap(label: string, options: { modifier?: boolean; unknown?: boolean } = {}) {
  const classes = ["keycap", options.modifier && "is-modifier", options.unknown && "is-unknown"].filter(Boolean).join(" ");
  return `<span class="${classes}" title="${options.unknown ? `未知按键：${escapeHtml(label)}` : escapeHtml(label)}">${escapeHtml(label)}</span>`;
}

export function HotkeyDisplay(keys: readonly string[], platform: HotkeyPlatform, options: { interactiveId?: string; status?: HotkeyState } = {}) {
  const keycaps = formatHotkeyForDisplay(keys, platform).map((key) => Keycap(key.label, key)).join("");
