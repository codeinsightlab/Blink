...
pub struct TrayControls { pub tray_icon: TrayIcon<tauri::Wry>, pub status_item: MenuItem<tauri::Wry>, pub toggle_item: MenuItem<tauri::Wry> }

pub fn install(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    let status_item = MenuItemBuilder::with_id("runtime-status", "● 正在监听").enabled(false).build(app)?;
    ...
    let menu = MenuBuilder::new(app).items(&[&status_item, &open_item, &toggle_item, &separator, &quit_item]).build()?;
    ...
    let tray_icon = TrayIconBuilder::with_id("keyflow-tray")
        ...
        .on_menu_event(|app, event| match event.id().as_ref() {
            "show-main-window" => { let _ = show_main_window(app); }
            "toggle-listener" => { let _ = toggle_listener(app); }
            "quit-keyflow" => { crate::quit_keyflow(app); }
            _ => {}
        })
    ...
    app.manage(TrayControls { tray_icon, status_item, toggle_item });
    Ok(())
}
...
pub fn update(app: &AppHandle, status: &str) {
    ...
    let _ = controls.toggle_item.set_text(if status == "PAUSED" { "恢复监听" } else { "暂停监听" });
}
